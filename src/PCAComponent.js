 ...
 *
 * @returns {Document Object} xml文档对象
 */
function loadXML() {
  ...
  if (typeof (oXmlDom.load) == 'undefined') {
    ...
    /*
     * when set async eq true and load local xml file,the chrome will throw exception
     * "XMLHttpRequest cannot load file:///...data.xml.
     ...
     *
     */
    xmlHttp.open("POST", "data.xml", false);
    xmlHttp.setRequestHeader('Content-Type', 'application/xml');
    xmlHttp.send(null);
    oXmlDom = xmlHttp.responseXML;
  } else {
    oXmlDom.load("data.xml");
  }
...
class HtmlSelect {
   ...
   * 显示select标签在页面中
   *
   * @param offset offset select元素父节点id
   */
  showHtml(offset) {
...
class LinkageMenu {
  ...
  init(xPath) {
    ...
    const xmlData = this.xml.documentElement.selectNodes(xPath);
  ...
  change() {
    // 获取onchange方法的绑定对象
    const oLink = LinkageMenu["link_" + this.id];

    oLink.htmlSelect.cleanOptions();

    // 替换xPath中的变量部分
    const sXpath = oLink.xPath.replace("&var", this.value);

    const xmlData = oLink.xml.documentElement.selectNodes(sXpath);

    for (let i = 0; i < xmlData.length; i++) {
      const _name = xmlData[i].getAttribute("name");
      oLink.htmlSelect.addOption(_name, _name);
    }
    const oLinkedLink = LinkageMenu["link_" + oLink.id];

    // 获取被绑定对象onchange绑定的对象
    if (oLinkedLink) {
      oLinkedLink.htmlSelect.cleanOptions();
      const data = xmlData[0].childNodes;
      for (let j = 0; j < data.length; j++) {
        if (data[j].nodeType === 1) { // ELEMENT_NODE
          const _name = data[j].getAttribute("name");
 ...
 * @returns {Province}
 */
export class Province extends LinkageMenu {
  constructor(sOffset, sId, sName, sVal) {
  ...
  linkCity(oCity) {
    ...
    if (this.val) {
      const sXpath = oCity.xPath.replace("&var", this.val);
      oCity.init(sXpath);
    }
  }
}
 ...
 * @returns {City}
 */
export class City extends LinkageMenu {

  constructor(sOffset, sId, sName, sVal, isInit, province) {
    super(sOffset, sId, sName, sVal);

    this.xPath = province
        ? "//province[@name='" + province + "']/city"
        : '//city';
  ...
  linkArea(oArea) {
    ...
    if (this.val) {
      ...
      const sXpath = oArea.xPath.replace("&var", this.val);
      oArea.init(sXpath);
    }
  }
}
 ...
 * @returns {Area}
 */
export class Area extends LinkageMenu {

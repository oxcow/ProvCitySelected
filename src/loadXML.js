export default function XmlDom() {
  if (window.ActiveXObject) {
    try {
      const oXmlDom = new ActiveXObject("Microsoft.XMLDOM");
      oXmlDom.async = false;
      return oXmlDom;
    ...
    const oXmlDom = document.implementation.createDocument("", "", null);
...
if (typeof (Document) != 'undefined') {
  ...
  Document.prototype.loadXML = function(sXml) {
    ...
    const oXmlDom = oParser.parseFromString(sXml, "text/xml");

    while (this.firstChild) {
      this.removeChild(this.firstChild);
    }
    for (let i = 0; i < oXmlDom.childNodes.length; i++) {
      const oNewNode = this.importNode(oXmlDom.childNodes[i], true);
  ...
  Element.prototype.selectNodes = function(sXPath) {
    const oEvaluator = new XPathEvaluator();
    const oResult = oEvaluator.evaluate(sXPath, this, null,
        XPathResult.ORDERED_NODE_ITERATOR_TYPE, null);
    const aNodes = [];
    if (oResult != null) {
      let oElement = oResult.iterateNext();
      while (oElement) {
